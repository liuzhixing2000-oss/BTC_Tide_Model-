#!/usr/bin/env python3
"""Crypto Tide V10.8.1 All-Signals Quality Display Live Engine."""
...
def parse_args():
    ...
    p.add_argument('--model-file',type=Path,default=Path('crypto_tide_engine_v10_8.py'))
    p.add_argument('--bundle-dir',type=Path,default=Path('v10_bundle'))
    p.add_argument('--runtime-dir',type=Path,default=Path(os.getenv('V10_RUNTIME_DIR','/tmp/tide_v10_8_runtime')))
    ...
    while True:
        ...
        try:
            if fingerprint(bundle_dir)!=initial:
                log('V10.8 bundle changed; restarting to load new research bundle.')
                os.execv(sys.executable,[sys.executable,*sys.argv])
        except Exception as exc: log('V10.6 bundle watcher error',repr(exc))

def main():
    a=parse_args()
    print('='*100,flush=True); print('CRYPTO TIDE V10.8 WATCH-TRACKING LIVE ENGINE',flush=True); print('='*100,flush=True)
    ...
    model.send_tg(
        '🟢 Crypto Tide V10.8 Stable Online\n\n'
