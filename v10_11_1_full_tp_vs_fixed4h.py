...
# -*- coding: utf-8 -*-
"""
V10.11.1 FULL-UNIVERSE TP vs FIXED-4H CHAMPIONSHIP
============================================
...
def parse_args():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("v10_11_1_full_tp_vs_fixed4h"),
    )
    ...
    p.add_argument(
        "--tp-grid",
        type=str,
        default="0.005,0.0075,0.01,0.0125,0.015,0.0175,0.02,0.0225,0.025,0.03,0.035,0.04,0.05,0.06,0.08,0.10",
        help="Take-profit levels as decimal returns. Example 0.02 = +2%.",
    )
    p.add_argument(
        "--max-hold-hours",
        type=float,
        default=48.0,
        help="Research safety cap. Position exits at this cap only if neither TP nor structure stop has triggered.",
    )
    p.add_argument(
        "--required-score",
        type=float,
...
def simulate_tp_structure(engine, df: pd.DataFrame, entry_idx: int, risk: dict, tp_pct: float, max_hold_hours: float):
    """
    Long trade:
    - entry at current V10.9 scored 15m candle close
    - keep the current V10.9 initial structural stop
    - keep the same confirmed swing-low stop ratchet
    - NO fixed 4h exit
    - take profit only when price reaches entry * (1 + tp_pct)
    - otherwise hold until structure stop is hit
    - max_hold_hours is only a research safety cap for finite backtests
    """
    ...
    target = entry * (1.0 + float(tp_pct))
    current_stop = hard
    highest = entry
    lowest = entry
    max_bars = max(1, int(round(float(max_hold_hours) * 4)))
    ...
    for idx in range(entry_idx + 1, last + 1):
        ...
        # if both target and stop are touched in the same 15m candle, count STOP first.
        if low <= current_stop:
            exit_price = current_stop
            reason = "dynamic_structure_stop" if current_stop > hard else "hard_structure_stop"
            return TradePath(
                exit_time=pd.Timestamp(row["open_time"]) + pd.Timedelta(minutes=15),
        ...
        if high >= target:
            return TradePath(
                exit_time=pd.Timestamp(row["open_time"]) + pd.Timedelta(minutes=15),
                exit_price=target,
                net_return=target / entry - 1 - float(engine.FEE_SLIPPAGE),
        ...
        # Same confirmed 5-bar swing-low ratchet as current V10.9 research.
        pivot = idx - 2
        if pivot >= entry_idx + 2 and pivot + 2 < len(df):
            plow = float(df.iloc[pivot]["low"])
            left_min = float(df.iloc[pivot - 2:pivot]["low"].min())
            right_min = float(df.iloc[pivot + 1:pivot + 3]["low"].min())
    ...
    return TradePath(
        exit_time=pd.Timestamp(row["open_time"]) + pd.Timedelta(minutes=15),
        exit_price=close,
        net_return=close / entry - 1 - float(engine.FEE_SLIPPAGE),
        exit_reason="research_time_cap",
...
def simulate_fixed4h_baseline(engine, df: pd.DataFrame, entry_idx: int, risk: dict):
    """Exact V10.9-style fixed 4h baseline with same structural-stop ratchet."""
    entry = float(df.iloc[entry_idx]["close"])
    atr = float(risk["atr"])
    hard = float(risk["hard_stop"])
    if not np.isfinite(hard) or hard >= entry or not np.isfinite(atr) or atr <= 0:
    ...
    highest = entry
    lowest = entry
    last = min(entry_idx + 16, len(df) - 1)
    ...
    for idx in range(entry_idx + 1, last + 1):
        ...
        if low <= current_stop:
            ...
            return TradePath(
                exit_time=pd.Timestamp(row["open_time"]) + pd.Timedelta(minutes=15),
                exit_price=current_stop,
                net_return=current_stop / entry - 1 - float(engine.FEE_SLIPPAGE),
                exit_reason=reason,
                bars_held=idx - entry_idx,
                mfe_pct=highest / entry - 1,
                mae_pct=lowest / entry - 1,
...
def collect_symbol_candidates(
    ...
    for entry_idx in scored_indices:
        ...
        # Need at least some future data. The simulator handles a finite research cap.
        ...
        for tp_pct in tp_grid:
            path = simulate_tp_structure(
                engine, df, int(entry_idx), risk, tp_pct, max_hold_hours
            )
            if path is None:
                continue
            ...
            out.append(
                {
                    "tp_pct": float(tp_pct),
                    "exit_method": f"TP_{tp_pct*100:.2f}PCT_STRUCTURE",
                    "symbol": symbol,
                    ...
                    "effective_account_risk_pct": float(sizing["effective_account_risk_pct"]),
                    "planned_loss_usdt": float(sizing["effective_planned_loss_usdt"]),
        ...
        # Fixed-4h control row on the exact same signal.
        base_path = simulate_fixed4h_baseline(engine, df, int(entry_idx), risk)
        if base_path is not None:
            out.append(
                {
                    "tp_pct": np.nan,
                    "exit_method": "BASE_FIXED_4H",
                    "symbol": symbol,
                    "entry_time": signal_close,
                    "exit_time": base_path.exit_time,
                    "grade": grade["grade"],
                    "grade_label": grade["label"],
                    "entry_price": float(latest["close"]),
                    "exit_price": base_path.exit_price,
                    "net_return": base_path.net_return,
                    "exit_reason": base_path.exit_reason,
                    "bars_held": base_path.bars_held,
                    "mfe_pct": base_path.mfe_pct,
                    "mae_pct": base_path.mae_pct,
                    "raw_quality": float(latest.get("raw_quality_score", np.nan)),
                    "next_quality": float(assessment["next_quality"]),
                    "combined_quality": float(assessment["combined_quality"]),
                    "confirmation_tests": int(assessment["confirmation_tests"]),
...
def main():
    ...
    tp_grid = sorted({
        float(x.strip()) for x in a.tp_grid.split(",") if x.strip()
    })
    if not tp_grid:
        raise ValueError("tp-grid is empty")
    if any(x <= 0 for x in tp_grid):
        raise ValueError("All TP levels must be > 0")
    ...
    print("V10.11.1 FULL-UNIVERSE TP vs FIXED-4H CHAMPIONSHIP", flush=True)
    print("=" * 100, flush=True)
    ...
    for exit_method in sorted(cand["exit_method"].unique()):
        ...
        for name, grades in policies:
            ...
            summary["tp_pct"] = (float(method_cand["tp_pct"].dropna().iloc[0]) if method_cand["tp_pct"].notna().any() else np.nan)
    ...
    # Direct candidate-level TP comparison on identical entries.
    tp_rows = []
    for (tp_pct, exit_method), g in cand[cand["exit_method"] != "BASE_FIXED_4H"].groupby(["tp_pct", "exit_method"]):
        r = g["net_return"].astype(float)
        ...
        tp_rows.append({
            ...
            "tp_hit_rate": float((g["exit_reason"] == "take_profit").mean()),
            "structure_stop_rate": float(
                g["exit_reason"].isin(["hard_structure_stop", "dynamic_structure_stop"]).mean()
            ),
            "time_cap_rate": float((g["exit_reason"] == "research_time_cap").mean()),
            "avg_mfe_pct": float(g["mfe_pct"].mean()),
            "avg_mae_pct": float(g["mae_pct"].mean()),
        })
    tp_comparison = pd.DataFrame(tp_rows).sort_values("tp_pct")
    tp_comparison.to_csv(a.output_dir / "tp_comparison.csv", index=False)

    # Grade-specific TP sweet spots.
    grade_tp_rows = []
    for (grade, tp_pct), g in cand[cand["exit_method"] != "BASE_FIXED_4H"].groupby(["grade", "tp_pct"]):
        r = g["net_return"].astype(float)
        gains = float(r[r > 0].sum())
        losses = float(-r[r < 0].sum())
        grade_tp_rows.append({
            "grade": grade,
            "tp_pct": float(tp_pct),
            "signals": int(len(g)),
            "avg_net_return": float(r.mean()),
    ...
    grade_tp = pd.DataFrame(grade_tp_rows).sort_values(["grade", "tp_pct"])
    grade_tp.to_csv(a.output_dir / "grade_tp_comparison.csv", index=False)

    # Apples-to-apples comparison against fixed 4h on the same signals.
    base_rows = cand[cand["exit_method"] == "BASE_FIXED_4H"][
        ["symbol", "entry_time", "grade", "net_return", "bars_held"]
    ].rename(columns={
        "net_return": "fixed4h_return",
        "bars_held": "fixed4h_bars",
    })

    delta_rows = []
    for tp_pct in sorted(cand["tp_pct"].dropna().unique()):
        g = cand[cand["tp_pct"] == tp_pct].merge(
            base_rows, on=["symbol", "entry_time", "grade"], how="inner"
        )
        if g.empty:
            continue
        ...
        delta_rows.append({
            "tp_pct": float(tp_pct),
            "signals": int(len(g)),
            "tp_avg_return": float(g["net_return"].mean()),
            ...
            "rescued_by_tp_structure_gt_0_5pct": int((d > 0.005).sum()),
            "gave_back_vs_fixed4h_gt_0_5pct": int((d < -0.005).sum()),
            "avg_hold_hours_tp": float(g["bars_held"].mean() / 4.0),
            "avg_hold_hours_fixed4h": float(g["fixed4h_bars"].mean() / 4.0),
        })
    tp_vs_fixed4h = pd.DataFrame(delta_rows).sort_values("tp_pct")
    tp_vs_fixed4h.to_csv(a.output_dir / "tp_vs_fixed4h.csv", index=False)
    ...
    print("\nTP SWEET-SPOT COMPARISON — SAME SIGNALS\n", flush=True)
    print(tp_comparison.to_string(index=False), flush=True)

    print("\nTP vs FIXED-4H — SAME SIGNALS\n", flush=True)
    print(tp_vs_fixed4h.to_string(index=False), flush=True)

    print("\nGRADE-SPECIFIC TP COMPARISON\n", flush=True)
    print(grade_tp.to_string(index=False), flush=True)
    ...
    report = [
        ...
        "TP SWEET-SPOT COMPARISON — SAME SIGNALS",
        "-" * 100,
        tp_comparison.to_string(index=False),
        "",
        "TP vs FIXED-4H — SAME SIGNALS",
        "-" * 100,
        tp_vs_fixed4h.to_string(index=False),
        "",
        "GRADE-SPECIFIC TP COMPARISON",
        "-" * 100,
        grade_tp.to_string(index=False),
        "",
        "GRADE SUMMARY BEFORE PORTFOLIO GATES",
        "-" * 100,
