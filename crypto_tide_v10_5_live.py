#!/usr/bin/env python3
"""Crypto Tide V10.5 Production Live Engine."""
...
def parse_args():
    ...
    p.add_argument('--model-file',type=Path,default=Path('crypto_tide_engine_v10_5.py'))
    p.add_argument('--bundle-dir',type=Path,default=Path('v10_bundle'))
    p.add_argument('--runtime-dir',type=Path,default=Path(os.getenv('V10_RUNTIME_DIR','/tmp/tide_v10_5_runtime')))
    ...
    while True:
        ...
        try:
            if fingerprint(bundle_dir)!=initial:
                log('V10.5 bundle changed; restarting to load new research bundle.')
                os.execv(sys.executable,[sys.executable,*sys.argv])
        except Exception as exc: log('V10.5 bundle watcher error',repr(exc))

def main():
    a=parse_args()
    print('='*100,flush=True); print('CRYPTO TIDE V10.5 PRODUCTION LIVE ENGINE',flush=True); print('='*100,flush=True)
