...
# ============================================================
# Crypto Tide Engine V10.8.1 Expectancy-Graded + Next Rescue Runtime
...
# V10.8.1 validated Next-candle Rescue path
...
GRADE_EXPECTANCY_REFERENCE = {
    ...
    "B": {
        ...
        "expectancy": 0.0000,
        "note": "Near-miss or mixed evidence; not a default production trade.",
    },
    "C": {
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V10.8.1 all-eligible watchlist refreshed\n\n"
...
def close_research_position(
    ...
    send_tg(f"""{exit_icon} Crypto Tide V10.8.1 {exit_title}: {symbol}
...
def send_quality_signal_alert(
    ...
    """V10.8: send every signal, grade it, and auto-track A-/A/A+."""
    signal_key = latest["open_time"].isoformat()
    ...
    entry_route = (
        ...
        else ("STRICT / STANDARD" if grade["actionable"] else "WATCH")
    )

    planned_exit = (
        "Fixed 4h (16 completed 15m bars), unless stop is hit first"
        if actionable
        else "WATCH tracking only; not a default production trade"
    )
...
Default system trade: {'YES' if actionable else 'NO'}
...
A  = recommended
A- = can take; strict base-system edge OR validated Next Rescue
B  = watch; positive edge not sufficiently established
C  = tested negative-expectancy context / avoid

Tracking:
• A-/A/A+ open a SYSTEM tracked position when portfolio capacity allows.
• B/C open a WATCH tracked position when no active position exists.
• Both classes receive stop/exit alerts.
• WATCH positions consume zero production portfolio risk.
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V10.8.1 found no robust-eligible symbols.\n"
