...
# ---------- V9 secondary confirmation ----------
# A raw liquidity-reclaim candle becomes tradable only if the following
# completed 15m candle confirms follow-through. At least two of the three
# quality tests must pass, and price must not invalidate the original signal.
SECONDARY_CONFIRMATION_ENABLED = True
SECONDARY_CONFIRMATION_MIN_TESTS = 2
CONFIRM_REQUIRE_BULLISH_CLOSE = True
CONFIRM_REQUIRE_CLOSE_ABOVE_SIGNAL_CLOSE = True
CONFIRM_REQUIRE_CLOSE_ABOVE_EMA20 = True
...
DEFAULT_EXIT_METHOD = "structure_fixed6"
...
BASE_MARGIN_USDT = float(os.getenv("BASE_MARGIN_USDT", "100"))
...
def model_frame(
    ...
    # V9: wait for the next completed 15m candle. This prevents a single
    # impressive-looking wick from becoming an entry when there is no follow-through.
    ...
    df["secondary_confirmation_pass"] = (
        previous_raw
        & confirmation_not_invalidated
        & (confirmation_tests >= SECONDARY_CONFIRMATION_MIN_TESTS)
    )
...
def strategy_spec(method: str) -> StrategySpec:
    # Backward compatibility: old saved configurations may still reference
    # the no-stop baseline. V9 transparently upgrades it to structure_fixed6.
    if method == "baseline_fixed6_no_stop":
        method = "structure_fixed6"
    try:
...
def run_exit_method(
    ...
    rolling_high_target = float(
        entry_row.get("confirmation_rolling_high", entry_row["rolling_high"])
    )
    if not np.isfinite(rolling_high_target):
    ...
    structure_buffer_atr, catastrophe_multiple_atr = (
        adaptive_stop_multipliers(entry, atr)
    )
    structure_stop = signal_low - structure_buffer_atr * atr
    catastrophe_stop = entry - catastrophe_multiple_atr * atr
...
def scan_and_select() -> tuple[list[str], pd.DataFrame]:
    ...
    lines = [
        "✅ Crypto Tide Engine V9 daily all-eligible learning complete",
        "",
    ...
    send_tg("\n".join(lines))
    send_tg_document(STAGE2_CSV, "V9 all-eligible symbol ranking")
    send_tg_document(EXIT_RESULTS_CSV, "V9 all exit-method results")
    send_tg_document(
        ENTRY_PARAM_RESULTS_CSV,
        "V9 bounded entry-parameter comparison",
    )
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V9 all-eligible watchlist refreshed\n\n"
...
def entry_risk_levels(latest: pd.Series) -> dict:
    ...
    structure_buffer_atr, catastrophe_multiple_atr = (
        adaptive_stop_multipliers(entry_price, atr)
    )
    structure_stop = signal_low - structure_buffer_atr * atr
    ...
    with position_lock:
        ...
        active_positions[symbol] = {
            ...
            "suggested_margin": float(
                BASE_MARGIN_USDT * risk_tier(score)[1]
            ),
...
def close_research_position(
    ...
    send_tg(f"""🏁 Crypto Tide V9 EXIT ALERT: {symbol}
...
def realtime_exit_update(symbol: str, latest: pd.Series) -> None:
    with position_lock:
        ...
        if (
            ...
            send_tg(f"""⚠️ Crypto Tide V9 LOGIC WARNING: {symbol}
...
def portfolio_capacity_status(proposed_margin: float) -> dict:
    """Check research-position capacity before a confirmed alert is admitted."""
    with position_lock:
        active = [
            position
        ...
        used_margin = sum(
            float(position.get("suggested_margin", BASE_MARGIN_USDT))
...
def send_live_alert(
    ...
    capacity = portfolio_capacity_status(suggested_margin)
    ...
    note = (
        ...
        if alert_type == "pre"
        else "The next 15m candle confirmed follow-through and the research position tracker has started."
    )
...
Secondary confirmation tests passed: {int(latest.get('secondary_confirmation_tests', 0))}/3
...
def calculate_live_signal(symbol: str, confirmed: bool) -> None:
    ...
    log(
        ...
        f"secondary_confirmed={confirmed_signal}",
    ...
    if confirmed:
        if confirmed_signal:
            ...
            log(symbol, "raw Tide candle closed; waiting for next-candle confirmation")
        return

    # Pre-alert remains attached to the still-open raw Tide candle. It is not
    # a trade instruction; the next completed candle must confirm the setup.
    if raw_signal and 0 <= minutes_to_close <= PRE_ALERT_WINDOW_MINUTES:
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V9 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V9 realtime monitor started\n\n"
