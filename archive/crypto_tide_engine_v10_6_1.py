...
# ============================================================
# Crypto Tide Engine V10.6.1 Stable Production/Research Dual-Signal Runtime
...
# ---------- V10.6.1 stable production filters ----------
# These thresholds were selected from the 45-day validation research.
# Signals that fail them are still sent as clearly marked RESEARCH alerts.
...
SEND_RESEARCH_ALERTS = (
    os.getenv("SEND_RESEARCH_ALERTS", "true").strip().lower()
    in {"1", "true", "yes", "on"}
)
PRODUCTION_EXIT_METHOD = os.getenv(
    "PRODUCTION_EXIT_METHOD", "structure_atr_trail_2.5"
)
ALLOW_DAILY_EXIT_SWITCH = (
    os.getenv("ALLOW_DAILY_EXIT_SWITCH", "false").strip().lower()
    in {"1", "true", "yes", "on"}
)
...
DEFAULT_EXIT_METHOD = "structure_atr_trail_2.5"
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V10.6 all-eligible watchlist refreshed\n\n"
...
def close_research_position(
    ...
    send_tg(f"""🏁 Crypto Tide V10.6 EXIT ALERT: {symbol}
...
def realtime_exit_update(symbol: str, latest: pd.Series) -> None:
    with position_lock:
        ...
        if spec.exit_mode=="fixed6" and bars>=24: exit_price,reason=close,"fixed_6h"
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V10.6 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V10.6.1 stable monitor started\n\n"
