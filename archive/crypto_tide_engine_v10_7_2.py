...
# ============================================================
# Crypto Tide Engine V10.7.2 All-Signals + Watch Position Tracking Runtime
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V10.7.2 all-eligible watchlist refreshed\n\n"
...
def close_research_position(
    ...
    send_tg(f"""{exit_icon} Crypto Tide V10.7.2 {exit_title}: {symbol}
...
def send_quality_signal_alert(
    ...
    """Send every completed scored setup with transparent pass/fail details.

    Returns True only when a system-approved production position was opened.
    """
    signal_key = latest["open_time"].isoformat()
    ...
    icon, label = classify_quality_signal(
        latest,
        score,
        assessment,
    )

    approved = bool(assessment["production"])
    capacity_note = ""
    ...
    if approved:
        ...
        if not capacity_allowed:
            approved = False
            icon = "🟡"
            label = "QUALITY PASSED / PORTFOLIO BLOCKED"
            capacity_note = (
    ...
    failure_lines = (
        ...
        else "• None — all production filters passed."
    )
    ...
    next_mark = quality_mark(
        next_quality,
        PRODUCTION_MIN_NEXT_QUALITY,
        near_margin=5.0,
    )
    combined_mark = quality_mark(
        combined_quality,
        PRODUCTION_MIN_COMBINED_QUALITY,
        near_margin=5.0,
    )
    confirmation_mark = (
        "✅ PASS"
    ...
    send_tg(f"""{icon} Tide SIGNAL: {symbol}

Overall: {label}
System entry: {'YES' if approved and capacity_allowed else 'NO'}
...
Hard-stop risk: {risk['risk_pct']:.2%} — {risk_mark}
Maximum reference: ≤ {PRODUCTION_MAX_HARD_STOP_RISK_PCT:.2%}
...
WHY NOT STRICTLY RECOMMENDED
{failure_lines}
{capacity_note}
Interpretation:
✅ PASS = meets the current strict reference
⚠️ NEAR = close to the strict reference
❌ FAIL = below the current strict reference

Tracking:
• Strictly approved signals open a SYSTEM tracked position.
• Non-approved signals open a WATCH tracked position when the symbol has
  no existing active tracked position.
• WATCH positions receive the same model stop/exit alerts.
• WATCH positions consume zero production portfolio risk and remain
  separate in the trade journal.

This message reports model quality; it is not an automatic exchange order.""")
    ...
    if approved and capacity_allowed:
    ...
    # Every non-production scored signal is opened as a zero-risk watch
    # position. It receives the same model exit/stop monitoring, but does
    # not consume production position count, margin, open risk, or daily
    # loss capacity.
    return open_research_position(
        symbol,
        latest,
...
def calculate_live_signal(symbol: str, confirmed: bool) -> None:
    ...
    if confirmed:
        ...
        if has_scored_setup:
            ...
            if assessment["production"] and position_opened:
                decision = "production_entry"
                reason = "all_strict_quality_filters_passed"
            elif assessment["production"]:
                decision = "qualified_but_not_opened"
                reason = (
                    "quality_passed_but_portfolio_blocked_or_existing_position"
                )
            elif position_opened:
                decision = "watch_position_opened"
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V10.7.2 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V10.7.2 watch-tracking monitor started\n\n"
