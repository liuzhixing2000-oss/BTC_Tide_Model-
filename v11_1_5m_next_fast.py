...
# -*- coding: utf-8 -*-
"""
V11.1 FAST 5m Next Confirmation
...
Quick:
  python v11_5m_entry_championship_v2.py --max-symbols 10

Full:
  python v11_5m_entry_championship_v2.py
"""
...
def parse_args():
    ...
    p.add_argument("--output-dir",type=Path,default=Path("v11_1_5m_next_fast"))
    ...
    p.add_argument("--entry-window-minutes",type=int,default=15)
...
def load_engine(path,data_dir):
    """Load a Tide engine from the repo without requiring an obsolete filename.

    Search order:
    1. --engine-file explicit/default path
    2. current production engine in repo root
    3. common archive locations
    4. legacy v10.6 fallback

    This keeps the V11 research service compatible with a cleaned repository.
    """
    candidates = [
        Path(path),
        ...
        Path("archive/crypto_tide_engine_v10_6.py"),
    ...
    print(f"Using Tide engine: {selected}")

    data_dir.mkdir(parents=True,exist_ok=True)
    os.environ["TIDE_DATA_DIR"]=str(data_dir.resolve())

    spec=importlib.util.spec_from_file_location("v11engine",selected)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot import {selected}")

    m=importlib.util.module_from_spec(spec)
    sys.modules[spec.name]=m
    spec.loader.exec_module(m)

    required = ["fetch_klines", "model_frame", "signal_components", "FEE_SLIPPAGE"]
    missing = [name for name in required if not hasattr(m, name)]
    if missing:
        raise RuntimeError(
            f"Engine {selected} is missing required research interfaces: {missing}"
        )
...
def main():
    ...
    rows=[]
    print("="*100); print("V11.1 FAST 5m NEXT CONFIRMATION"); print("Symbols:",len(u)); print("FAST defaults: 45d 5m / 60d 15m / 90d 1h")
    ...
    t.to_csv(a.output_dir/"v11_1_5m_next_trades.csv",index=False)
    ...
    rank.to_csv(a.output_dir/"v11_1_5m_next_rankings.csv",index=False)
    ...
    report="V11.1 FAST 5m NEXT CONFIRMATION\n"+"="*100+"\n"
    ...
    (a.output_dir/"v11_1_5m_next_report.txt").write_text(report,encoding="utf-8")
