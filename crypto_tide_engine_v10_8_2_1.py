...
# ============================================================
# Crypto Tide Engine V10.8.2.1 Maintenance Runtime
...
def maybe_send_daily_health() -> None:
    ...
    # The monitor loop runs every 60s. Allow the report any time from 07:55
    # onward so a delayed loop still reports before/rearound the daily restart.
    report_window = (
        (local_now.hour == 7 and local_now.minute >= 55)
        or local_now.hour >= 8
    )
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V10.8.2.1 all-eligible watchlist refreshed\n\n"
...
def close_research_position(
    ...
    send_tg(f"""{exit_icon} Crypto Tide V10.8.2.1 {exit_title}: {symbol}
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V10.8.2.1 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V10.8.2.1 maintenance monitor started\n\n"
