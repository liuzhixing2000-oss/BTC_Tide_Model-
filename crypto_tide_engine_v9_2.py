...
def scan_and_select() -> tuple[list[str], pd.DataFrame]:
    ...
    lines = [
        "✅ Crypto Tide Engine V9.2 daily all-eligible learning complete",
        "",
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V9.2 all-eligible watchlist refreshed\n\n"
...
def close_research_position(
    ...
    send_tg(f"""🏁 Crypto Tide V9.2 EXIT ALERT: {symbol}
...
def realtime_exit_update(symbol: str, latest: pd.Series) -> None:
    with position_lock:
        ...
        if (
            ...
            send_tg(f"""⚠️ Crypto Tide V9.2 LOGIC WARNING: {symbol}
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V9.2 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V9.2 realtime monitor started\n\n"
