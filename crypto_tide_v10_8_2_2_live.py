#!/usr/bin/env python3
"""Crypto Tide V10.8.2.2 All-Signals Quality Display Live Engine."""
...
def parse_args():
    ...
    p.add_argument('--model-file',type=Path,default=Path('crypto_tide_engine_v10_8_2_2.py'))
    p.add_argument('--bundle-dir',type=Path,default=Path('v10_bundle'))
    p.add_argument('--runtime-dir',type=Path,default=Path(os.getenv('V10_RUNTIME_DIR','/data/tide_v10_8_2_2_runtime')))
...
def prepare_runtime(bundle_dir,runtime_dir):
    """Refresh bundle inputs without deleting learned/live state.

    V10.8.2.1 cleared the whole runtime directory on every 6-hour exec restart,
    which also erased daily_health_state.json and made the health counters
    restart from zero. Only bundle-owned files are overwritten now.
    """
    runtime_dir.mkdir(parents=True,exist_ok=True)
    for p in bundle_dir.iterdir():
        if p.is_file() and p.name!='manifest.json':
            shutil.copy2(p,runtime_dir/p.name)
    ...
    while True:
        ...
        try:
            if fingerprint(bundle_dir)!=initial:
                log('V10.8.2.2 bundle changed; restarting to load new research bundle.')
                os.execv(sys.executable,[sys.executable,*sys.argv])
        except Exception as exc: log('V10.6 bundle watcher error',repr(exc))

def main():
    a=parse_args()
    print('='*100,flush=True); print('CRYPTO TIDE V10.8.2.2 B+ MINIMUM TELEGRAM LIVE ENGINE',flush=True); print('='*100,flush=True)
    ...
    model.send_tg(
        '🟢 Crypto Tide V10.8.2.2 Stable Online\n\n'
