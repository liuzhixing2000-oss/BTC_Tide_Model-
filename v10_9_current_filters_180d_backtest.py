...
# -*- coding: utf-8 -*-
"""
V10.9 CURRENT-FILTER 180D PORTFOLIO BACKTEST
============================================
...
def parse_args():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("v10_9_180d_backtest"),
    )
...
def simulate_fixed4h_live_style(engine, df: pd.DataFrame, entry_idx: int, risk: dict):
    """
    Mirrors current live production tracking closely:
    - entry at completed 15m scored candle close
    - initial stop = V10.9 entry_risk_levels hard stop
    - confirmed swing-low stop ratchet
    - fixed exit after 16 completed 15m bars
    - fee/slippage deducted once using engine FEE_SLIPPAGE
    """
    entry = float(df.iloc[entry_idx]["close"])
    atr = float(risk["atr"])
    hard = float(risk["hard_stop"])
    if not np.isfinite(hard) or hard >= entry or atr <= 0:
        return None
    ...
    for idx in range(entry_idx + 1, last + 1):
        ...
        # Stop is checked before the new swing-low ratchet, matching live flow.
        if low <= current:
            exit_price = current
            bars = idx - entry_idx
            return TradePath(
                ...
                exit_reason=(
                    "dynamic_structure_stop" if current > hard
                    else "hard_structure_stop"
                ),
                bars_held=bars,
        ...
        # Historical equivalent of the live 5-bar confirmed pivot rule.
        pivot = idx - 2
...
def collect_symbol_candidates(
    ...
    for entry_idx in scored_indices:
        ...
        path = simulate_fixed4h_live_style(engine, df, int(entry_idx), risk)
        if path is None:
            continue
...
def main():
    ...
    print("V10.9 CURRENT-FILTER 180D PORTFOLIO BACKTEST", flush=True)
    print("=" * 100, flush=True)
    ...
    for name, grades in policies:
        trades, blocked = portfolio_sim(
            engine, cand, grades, capital, name
        )
        if not trades.empty:
            all_trades.append(trades)
        summaries.append(
            summarize_policy(trades, capital, blocked, name)
        )
    ...
    grade_rows = []
    for grade, g in cand.groupby("grade"):
    ...
    grade_summary = pd.DataFrame(grade_rows).sort_values(
        "grade",
        key=lambda s: s.map({"B+": 0, "A-": 1, "A": 2, "A+": 3}),
    )
