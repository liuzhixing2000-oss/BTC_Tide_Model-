...
# -*- coding: utf-8 -*-
"""
V10.10 EXIT-EXTENSION 180D BACKTEST
============================================
...
def parse_args():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("v10_10_exit_extension_backtest"),
    )
...
def _trade_path(engine, df, entry_idx, exit_idx, exit_price, entry, highest, lowest, reason):
    return TradePath(
        exit_time=pd.Timestamp(df.iloc[exit_idx]["open_time"]) + pd.Timedelta(minutes=15),
...
def _indicators_for_exit(df: pd.DataFrame):
...
def simulate_exit_method(engine, df: pd.DataFrame, entry_idx: int, risk: dict, method: str):
    """
    Same entry + initial/ratcheting structural stop for every method.
    Only the time/extension exit differs.

    Methods:
      BASE_FIXED_4H:
          exit at 16 completed 15m bars.
      FIXED_8H:
          exit at 32 completed 15m bars.
      COND_4H_TO_8H:
          at/after 4h, continue while at least 2 of 3 are healthy:
            1) close > EMA20
            2) MACD histogram >= 0
            3) short structure is not breaking down:
               current close >= minimum low of prior 4 completed bars
          check every 15m; hard cap at 8h.
      TRAIL_4H_TO_8H:
          before 4h use the normal structural stop.
          after 4h activate a 1.5 ATR trailing stop from highest high,
          while retaining the tighter confirmed structural stop.
          hard cap at 8h.
    """
    ...
    ema20, macd_hist = _indicators_for_exit(df)

    current = hard
    highest = entry
    lowest = entry

    max_bars = 16 if method == "BASE_FIXED_4H" else 32
    ...
    for idx in range(entry_idx + 1, last + 1):
        ...
        # For the trailing method, the trailing stop becomes active only after 4h.
        effective_stop = current
        if method == "TRAIL_4H_TO_8H" and idx - entry_idx > 16:
            effective_stop = max(effective_stop, highest - 1.50 * atr)
            # Never place a long stop above the current close in this bar-level simulation.
            effective_stop = min(effective_stop, close - 1e-12)

        # Existing stop is checked before adding a newly confirmed pivot.
        if low <= effective_stop:
            return _trade_path(
                engine, df, entry_idx, idx, effective_stop, entry,
                highest, lowest,
                "extension_trailing_stop" if effective_stop > current
                else ("dynamic_structure_stop" if current > hard else "hard_structure_stop")
            )

        # Same historical 5-bar confirmed pivot ratchet as V10.9.
        pivot = idx - 2
        if pivot >= entry_idx + 2 and pivot + 2 < len(df):
            ...
            if plow < left_min and plow <= right_min:
                ...
                if candidate > current and candidate < close:
                    current = candidate

        held = idx - entry_idx

        if method == "BASE_FIXED_4H" and held >= 16:
            return _trade_path(
                engine, df, entry_idx, idx, close, entry, highest, lowest, "fixed_4h"
            )

        if method == "FIXED_8H" and held >= 32:
            return _trade_path(
                engine, df, entry_idx, idx, close, entry, highest, lowest, "fixed_8h"
            )

        if method == "COND_4H_TO_8H" and held >= 16:
            # Evaluate extension health only from information available at this close.
            ema_ok = np.isfinite(float(ema20.iloc[idx])) and close > float(ema20.iloc[idx])
            macd_ok = np.isfinite(float(macd_hist.iloc[idx])) and float(macd_hist.iloc[idx]) >= 0

            prior_start = max(entry_idx + 1, idx - 4)
            prior = df.iloc[prior_start:idx]
            if len(prior) > 0:
                prior_floor = float(prior["low"].astype(float).min())
                structure_ok = close >= prior_floor
            else:
                structure_ok = True

            health = int(ema_ok) + int(macd_ok) + int(structure_ok)

            if held >= 32:
                return _trade_path(
                    engine, df, entry_idx, idx, close, entry, highest, lowest,
                    "conditional_8h_cap"
                )
            if health < 2:
                return _trade_path(
                    engine, df, entry_idx, idx, close, entry, highest, lowest,
                    "conditional_extension_fail"
                )

        if method == "TRAIL_4H_TO_8H" and held >= 32:
            return _trade_path(
                engine, df, entry_idx, idx, close, entry, highest, lowest,
                "trailing_8h_cap"
            )
    ...
    close = float(row["close"])
    return _trade_path(
        engine, df, entry_idx, last, close, entry, highest, lowest, "data_end"
    )
...
def collect_symbol_candidates(
    ...
    for entry_idx in scored_indices:
        ...
        # Need 16 future bars for a complete fixed-4h result.
        if entry_idx + 16 >= len(df):
            continue
        ...
        for exit_method in EXIT_METHODS:
            path = simulate_exit_method(engine, df, int(entry_idx), risk, exit_method)
            if path is None:
                continue
...
def main():
    ...
    print("V10.10 EXIT-EXTENSION 180D BACKTEST", flush=True)
    print("=" * 100, flush=True)
    ...
    # Direct apples-to-apples candidate-level comparison.
    comparison_rows = []
    base = cand[cand["exit_method"] == "BASE_FIXED_4H"][
        ["symbol", "entry_time", "grade", "net_return", "exit_price"]
    ].rename(columns={"net_return": "base_return", "exit_price": "base_exit_price"})

    for method in EXIT_METHODS:
        g = cand[cand["exit_method"] == method].copy()
        merged = g.merge(base, on=["symbol", "entry_time", "grade"], how="inner")
        if merged.empty:
            continue
        delta = merged["net_return"].astype(float) - merged["base_return"].astype(float)
        comparison_rows.append({
            "exit_method": method,
            "signals": int(len(merged)),
            "avg_net_return": float(merged["net_return"].mean()),
            "median_net_return": float(merged["net_return"].median()),
            "win_rate": float((merged["net_return"] > 0).mean()),
            "avg_bars_held": float(merged["bars_held"].mean()),
            "avg_mfe_pct": float(merged["mfe_pct"].mean()),
            "avg_mae_pct": float(merged["mae_pct"].mean()),
            "avg_delta_vs_fixed4h": float(delta.mean()),
            "rescued_vs_fixed4h": int((delta > 0.005).sum()),
            "gave_back_vs_fixed4h": int((delta < -0.005).sum()),
            "pct_better_than_fixed4h": float((delta > 0).mean()),
        })
