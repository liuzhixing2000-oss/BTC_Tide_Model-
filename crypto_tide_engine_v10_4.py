...
# ============================================================
# Crypto Tide Engine V10.4 Runtime Strategy.2
...
def scan_and_select() -> tuple[list[str], pd.DataFrame]:
    ...
    lines = [
        "✅ Crypto Tide Engine V10.4 daily all-eligible learning complete",
        "",
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V10.4 all-eligible watchlist refreshed\n\n"
...
def open_research_position(
    ...
    with position_lock:
        ...
        active_positions[symbol] = {
            ...
            "suggested_leverage": DEFAULT_LEVERAGE,
            "suggested_notional": float(
                BASE_MARGIN_USDT * risk_tier(score)[1] * DEFAULT_LEVERAGE
            ),
...
def close_research_position(
    ...
    send_tg(f"""🏁 Crypto Tide V10.4 EXIT ALERT: {symbol}
...
def realtime_exit_update(symbol: str, latest: pd.Series) -> None:
    with position_lock:
        ...
        if (
            ...
            send_tg(f"""⚠️ Crypto Tide V10.4 LOGIC WARNING: {symbol}
...
def send_live_alert(
    ...
    suggested_notional = suggested_margin * DEFAULT_LEVERAGE
    ...
    title = (
        ...
        if alert_type == "pre"
        else f"🟢 Tide CONFIRMED ENTRY: {symbol}"
    )
    note = (
...
Suggested margin: {suggested_margin:.0f} USDT
Suggested leverage: {DEFAULT_LEVERAGE:.0f}x
...
def calculate_live_signal(symbol: str, confirmed: bool) -> None:
    ...
    if (
        SEND_PRE_ALERTS
        and raw_signal
        and 0 <= minutes_to_close <= PRE_ALERT_WINDOW_MINUTES
    ):
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V10.4 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V10.4 realtime monitor started\n\n"
        ...
        f"Displayed leverage: {DEFAULT_LEVERAGE:.0f}x\n"
