#!/usr/bin/env python3
"""Crypto Tide V10.4 Production Live Engine."""
...
def parse_args():
    ...
    p.add_argument('--model-file',type=Path,default=Path('crypto_tide_engine_v10_4.py'))
    p.add_argument('--bundle-dir',type=Path,default=Path('v10_bundle'))
    p.add_argument('--runtime-dir',type=Path,default=Path(os.getenv('V10_RUNTIME_DIR','/tmp/tide_v10_4_runtime')))
    ...
    while True:
        ...
        try:
            if fingerprint(bundle_dir)!=initial:
                log('V10.4 bundle changed; restarting to load new research bundle.')
                os.execv(sys.executable,[sys.executable,*sys.argv])
        except Exception as exc: log('V10.4 bundle watcher error',repr(exc))

def main():
    a=parse_args()
    print('='*100,flush=True); print('CRYPTO TIDE V10.4 PRODUCTION LIVE ENGINE',flush=True); print('='*100,flush=True)
    ...
    model.send_tg(
        ...
        f'Pre-alert Telegram: {"ON" if model.SEND_PRE_ALERTS else "OFF"}\n'
        f'Minimum raw quality: {model.MIN_RAW_QUALITY_SCORE:.0f}\n'
        f'Minimum combined setup: {model.MIN_COMBINED_SETUP_SCORE:.0f}\n'
