...
- Railway GraphQL returns the timestamp OUTSIDE `message`; most messages begin
  directly with SYMBOLUSDT. Parser uses the Railway row timestamp first.
- Railway stdout can interleave multiple threads, so each message is segmented
  around every SYMBOLUSDT anchor.
...
REPORT_VERSION = "v4-railway-row-timestamp"
...
def project_services(t,pid):
    ...
    p=gql(t,q,{"id":pid})["project"]; return p["name"],[x["node"] for x in p["services"]["edges"]]
def deployments(t,pid,sid,eid,limit=1000):
    ...
    d=gql(t,q,{"input":{"projectId":pid,"serviceId":sid,"environmentId":eid},"first":limit}); return [x["node"] for x in d["deployments"]["edges"]]
def deployment_logs(t,did,limit=1000):
    ...
    return gql(t,q,{"deploymentId":did,"limit":limit})["deploymentLogs"]
def month_bounds(m):
...
# Message may optionally contain an internal timestamp, but Railway API normally
# supplies timestamp separately and message starts with e.g. `FILUSDT confirmed=...`.
...
def field(name,text):
    ...
    if name in("raw_quality","confirmation_quality","combined_setup"):return None if x.lower()=="nan" else float(x)
    return x

def segments(msg):
    text=str(msg or ""); aa=list(ANCHOR.finditer(text)); out=[]
    for i,a in enumerate(aa):
        end=aa[i+1].start() if i+1<len(aa) else len(text); c=text[a.start():end]
...
def make_event(a,c,row_ts):
    internal=a.group("ts"); t=dt(internal) if internal else dt(row_ts)
    e={"timestamp":t,"symbol":a.group("symbol")}
    for k in FIELD:e[k]=field(k,c)
    gm=GRADE_RE.search(c);e["explicit_grade"]=gm.group(1) if gm else None
    sm=SIDE_RE.search(c);e["side"]=("LONG" if sm.group(1).upper() in("LONG","BUY") else "SHORT") if sm else "LONG"
    return e

def parse_events(rows,start,end):
    d=Counter(); cand=[]; ordered=sorted(rows,key=lambda x:str(x.get("timestamp","")))
    for row in ordered:
        msg=str(row.get("message",""));d["rows"]+=1
        d["confirmed_true_literals"]+=len(re.findall(r"(?<!score_)confirmed\s*=\s*True",msg))
        d["score_confirmed_true_literals"]+=len(re.findall(r"\bscore_confirmed\s*=\s*True",msg))
        d["raw_signal_true_literals"]+=len(re.findall(r"\braw_signal\s*=\s*True",msg))
        seg=segments(msg);d["segments"]+=len(seg)
        for a,c in seg:
            ...
            # exact production alert timing: closed candle + model signal true
            if e.get("confirmed") is True and e.get("score_confirmed") is True:
                e["grade"],e["grade_inferred"]=infer_grade(e);cand.append(e);d["qualified_before_dedupe"]+=1
    uniq={}
    for e in cand:
        candle=e["timestamp"].replace(minute=e["timestamp"].minute//15*15,second=0,microsecond=0);key=(e["symbol"],candle)
        old=uniq.get(key);rich=sum(v is not None for v in e.values());oldrich=sum(v is not None for v in old.values()) if old else -1
        if rich>oldrich:uniq[key]=e
    d["unique_events"]=len(uniq);return sorted(uniq.values(),key=lambda x:x["timestamp"]),d

def parser_self_test():
    row={"timestamp":"2026-08-10T12:15:01.000Z","message":"FILUSDT confirmed=True regime=downtrend raw_signal=True score_confirmed=True raw_quality=70 confirmation_quality=96 combined_setup=76 confirmation_tests=3"}
    ev,d=parse_events([row],datetime(2026,8,1,tzinfo=timezone.utc),datetime(2026,9,1,tzinfo=timezone.utc))
    assert len(ev)==1 and ev[0]["symbol"]=="FILUSDT" and ev[0]["side"]=="LONG",(ev,d)
...
def bybit_15m(sym,start,hours=8):
    ...
    r=requests.get(BYBIT_KLINE,params={"category":"linear","symbol":sym,"interval":"15","start":s,"end":e,"limit":1000},timeout=30);r.raise_for_status();j=r.json()
...
def main():
    parser_self_test();print("Report parser:",REPORT_VERSION,"self-test=PASS")
    ...
    start,end=month_bounds(a.month);end=min(end,datetime.now(timezone.utc));ctx=token_context(token);pn,ss=project_services(token,ctx["projectId"]);print("Railway project:",pn);print("Services:",[(s["name"],s["id"]) for s in ss])
    targets=[s for s in ss if s["id"]==a.service_id] if a.service_id else ([s for s in ss if "tide" in s["name"].lower()] or ss)
    ...
    for svc in targets:
        ...
        for n,d in enumerate(chosen,1):
            try:
                rows=deployment_logs(token,d["id"],1000);ok+=1
                for r in rows:r["deployment_id"]=d["id"];r["deployment_status"]=d["status"];r["service_name"]=svc["name"]
                allrows.extend(rows);print(n,d["id"],d["status"],len(rows))
            except Exception as ex:fail+=1;print("WARN",d["id"],repr(ex))
    raw=out/f"railway_logs_{a.month}.jsonl"
    with raw.open("w",encoding="utf-8") as f:
        for r in allrows:f.write(json.dumps(r,ensure_ascii=False,default=str)+"\n")
    ev,diag=parse_events(allrows,start,end);diag.update({"report_version":REPORT_VERSION,"deployments":dc,"log_fetch_ok":ok,"log_fetch_failed":fail,"log_rows":len(allrows)})
    (out/f"parser_diagnostics_{a.month}.json").write_text(json.dumps(dict(diag),indent=2,default=str),encoding="utf-8")
    print("PARSER DIAGNOSTICS",json.dumps(dict(diag),sort_keys=True));print("Unique qualified events:",len(ev))
    measured=[]
    ...
    if df.empty:lines+=["No closed-candle + score_confirmed=True production signal was reconstructed. See parser_diagnostics and railway_logs artifacts before interpreting this as a model result."]
    else:
        ...
        for grade,g in df.groupby("grade",dropna=False):
            ...
            lines.append(f"- {grade}: signals={len(g)}, 4h mean={pct(x.mean() if len(x) else None)}, 4h win={(x>0).mean()*100:.1f}%" if len(x) else f"- {grade}: signals={len(g)}, 4h unavailable")
