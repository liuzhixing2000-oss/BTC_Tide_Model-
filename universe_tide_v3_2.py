...
def send_tg_photo(image_bytes, caption):
    ...
    try:
        resp=requests.post(f"https://api.telegram.org/bot{token}/sendPhoto",
            ...
            files={"photo":("tide_ranking.png",image_bytes,"image/png")},timeout=40)
        log("Telegram photo",resp.status_code,resp.text[:200])
    except Exception as e: log("Telegram photo failed",repr(e))


def send_tg_document(path, caption):
    token=os.getenv("TELEGRAM_BOT_TOKEN","").strip(); chat_id=os.getenv("TELEGRAM_CHAT_ID","").strip()
    if not token or not chat_id or not Path(path).exists(): return
    try:
        with open(path,"rb") as f:
            resp=requests.post(f"https://api.telegram.org/bot{token}/sendDocument",
                data={"chat_id":chat_id,"caption":caption},
                files={"document":(Path(path).name,f,"text/csv")},timeout=60)
        log("Telegram CSV",resp.status_code,resp.text[:200])
    except Exception as e: log("Telegram CSV failed",repr(e))


def ranking_chart(results, top_n=12):
    ranked=results.head(top_n).copy()
    labels=ranked.symbol.tolist()[::-1]
    vals=(ranked.total_return*100).tolist()[::-1]
    fig=Figure(figsize=(10,7),dpi=150); ax=fig.subplots(); y=np.arange(len(labels))
    ax.barh(y,vals); ax.set_yticks(y,labels); ax.set_xlabel("60-day backtest return (%)")
    ax.set_title("Tide Universe — selected ranking"); ax.grid(axis="x",alpha=.25)
    for i,v in enumerate(vals):
        ax.text(v+(.25 if v>=0 else -.25),i,f"{v:.1f}%",va="center",ha="left" if v>=0 else "right",fontsize=9)
    fig.tight_layout(); buf=BytesIO(); fig.savefig(buf,format="png",bbox_inches="tight"); buf.seek(0); return buf


def send_visual_report(results, selected):
    try: send_tg_photo(ranking_chart(results,12),"Tide Universe top 12. Ranking is shown by 60-day return; live selection also uses stability filters.")
    except Exception as e: log("Chart failed",repr(e))
    send_tg_document(RESULTS_CSV,"Full 60-coin Tide backtest results")
    selected_rows=results[results.symbol.isin(selected)].sort_values(["score","total_return"],ascending=False,na_position="last")
    lines=["🏆 Selected-symbol metrics",""]
    for x in selected_rows.itertuples(index=False):
        score="n/a" if pd.isna(x.score) else f"{x.score:.3f}"
        lines += [x.symbol,f"Return: {x.total_return:.1%}",f"Trades: {x.trades}",f"Win rate: {x.win_rate:.1%}",f"Median: {x.median_return:.2%}",f"Max DD: {x.max_drawdown:.1%}",f"Score: {score}",""]
    text="\n".join(lines)
    for i in range(0,len(text),3500): send_tg(text[i:i+3500])
