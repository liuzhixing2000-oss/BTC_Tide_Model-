...
def main():
    ...
    for n, symbol in enumerate(
        ...
        for i in range(len(df15) - 1):
            ...
            # Compare timezone-aware pandas timestamps directly.
            # Using NumPy datetime64 here can coerce the tz-aware series to
            # object/int on some pandas/NumPy versions (e.g. Railway), causing
            # Timestamp-vs-int TypeError.
            candidate_ids = np.flatnonzero(
                (df5["close_time"] > setup_close_time).to_numpy()
            )
            ...
            base_ids = np.flatnonzero(
                (df5["close_time"] >= next15_close_time).to_numpy()
            )
            if len(base_ids) == 0:
                continue
