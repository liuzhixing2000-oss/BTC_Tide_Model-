...
# -*- coding: utf-8 -*-
"""
V10.13 ACTIVE-TIME EXIT CHAMPIONSHIP
============================================
...
def parse_args():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("v10_13_active_time_exit"),
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=24.0,
        help="Safety cap for active-time exits. Default 24 wall-clock hours.",
    )
    p.add_argument(
        "--active-hours-required",
        type=int,
        default=4,
        help="Number of qualifying 1h candles required before exit. Default 4.",
    )
    p.add_argument(
        "--required-score",
        type=float,
...
def prepare_hourly_activity(df15: pd.DataFrame) -> pd.DataFrame:
    """
    Build standard clock-hour candles from 15m data, using only completed 1h bars.
    Rolling reference medians are shifted by one hour to avoid look-ahead.
    """
    x = df15.copy()
    x["open_time"] = pd.to_datetime(x["open_time"], utc=True)
    x = x.sort_values("open_time").reset_index(drop=True)
    x["hour"] = x["open_time"].dt.floor("h")

    agg = x.groupby("hour", as_index=False).agg(
        open=("open", "first"),
        high=("high", "max"),
        low=("low", "min"),
        close=("close", "last"),
    ...
    agg = agg[agg["bars"] >= 4].copy()

    agg["range"] = agg["high"].astype(float) - agg["low"].astype(float)
    agg["movement"] = (agg["close"].astype(float) - agg["open"].astype(float)).abs()

    # 20-hour rolling medians using PREVIOUS completed hours only.
    agg["vol_med20"] = agg["volume"].astype(float).rolling(20, min_periods=10).median().shift(1)
    agg["range_med20"] = agg["range"].astype(float).rolling(20, min_periods=10).median().shift(1)
    agg["move_med20"] = agg["movement"].astype(float).rolling(20, min_periods=10).median().shift(1)
    agg["close_time"] = agg["hour"] + pd.Timedelta(hours=1)
    return agg


def active_hour_qualifies(row: pd.Series, method: str) -> bool:
    v = float(row["volume"])
    vm = float(row["vol_med20"]) if np.isfinite(row["vol_med20"]) else np.nan
    r = float(row["range"])
    rm = float(row["range_med20"]) if np.isfinite(row["range_med20"]) else np.nan
    m = float(row["movement"])
    mm = float(row["move_med20"]) if np.isfinite(row["move_med20"]) else np.nan
    ...
    if method == "VOL_RANGE":
        return (
            np.isfinite(rm) and rm > 0
            and v >= 1.00 * vm
            and r >= 0.75 * rm
        )
    if method == "VOL_MOVEMENT":
        return (
            np.isfinite(mm) and mm > 0
            and v >= 1.00 * vm
            and m >= 0.50 * mm
        )
    return False
...
def simulate_active_time_exit(
    engine,
    df: pd.DataFrame,
    entry_idx: int,
    risk: dict,
    method: str,
    active_hours_required: int,
    ...
    BASE_FIXED_4H:
        current control, exit after 16 completed 15m bars.

    VOL_075 / VOL_100 / VOL_125 / VOL_150:
        wall-clock hours do NOT count automatically.
        A standard completed 1h candle counts only when its volume reaches
        the chosen multiple of the previous-20h median volume.
        Exit when N active hours have completed.

    VOL_RANGE:
        active if 1h volume >= prior-20h median AND
        1h range >= 0.75 * prior-20h median range.

    VOL_MOVEMENT:
        active if 1h volume >= prior-20h median AND
        absolute open-to-close movement >= 0.50 * prior-20h median movement.

    Non-baseline methods have a max_wall_hours safety cap (default 24h).
    """
    ...
    hourly = prepare_hourly_activity(df)
    entry_time = pd.Timestamp(df.iloc[entry_idx]["open_time"]) + pd.Timedelta(minutes=15)
    ...
    if method == "BASE_FIXED_4H":
        ...
        last = min(entry_idx + int(round(max_wall_hours * 4)), len(df) - 1)

    processed_hour_closes = set()
    ...
    for idx in range(entry_idx + 1, last + 1):
        ...
        # Same confirmed 5-bar pivot stop ratchet as V10.9.
        pivot = idx - 2
        ...
        now_close = pd.Timestamp(row["open_time"]) + pd.Timedelta(minutes=15)
        ...
        # Count standard completed 1h candles only once, and only if their full hour
        # completed after entry.
        eligible = hourly[
            (hourly["close_time"] > entry_time)
            & (hourly["close_time"] <= now_close)
        ]
        if not eligible.empty:
            for _, hr in eligible.iterrows():
                key = pd.Timestamp(hr["close_time"])
                if key in processed_hour_closes:
                    continue
                processed_hour_closes.add(key)
                if active_hour_qualifies(hr, method):
                    active_count += 1

        if active_count >= active_hours_required:
            return TradePath(
                exit_time=now_close,
                exit_price=close,
                net_return=close / entry - 1 - float(engine.FEE_SLIPPAGE),
                exit_reason=f"{method.lower()}_{active_hours_required}_active_hours",
                bars_held=held_bars,
                mfe_pct=highest / entry - 1,
                mae_pct=lowest / entry - 1,
            )
...
def collect_symbol_candidates(
    ...
    for entry_idx in scored_indices:
        ...
        for exit_method in EXIT_METHODS:
            path = simulate_active_time_exit(
                engine,
                df,
                int(entry_idx),
                risk,
                exit_method,
                active_hours_required,
                max_wall_hours,
            )
            if path is None:
                continue
...
def main():
    ...
    print("V10.13 ACTIVE-TIME EXIT CHAMPIONSHIP", flush=True)
    print("=" * 100, flush=True)
    ...
    for method, g in cand.groupby("exit_method"):
        ...
        exit_rows.append({
            ...
            "median_hours_held": float(g["bars_held"].median() / 4.0),
    ...
    exit_comparison.to_csv(a.output_dir / "active_time_comparison.csv", index=False)
    ...
    for method in EXIT_METHODS:
        ...
        delta_rows.append({
            ...
            "avg_fixed4h_hold_hours": float(g["fixed4h_bars"].mean() / 4.0),
        })
    vs_fixed4h = pd.DataFrame(delta_rows).sort_values("avg_delta_vs_fixed4h", ascending=False)
    vs_fixed4h.to_csv(a.output_dir / "active_time_vs_fixed4h.csv", index=False)
    ...
    print("\nACTIVE-TIME EXIT COMPARISON — SAME SIGNALS\n", flush=True)
    print(exit_comparison.to_string(index=False), flush=True)

    print("\nACTIVE-TIME vs FIXED-4H — SAME SIGNALS\n", flush=True)
