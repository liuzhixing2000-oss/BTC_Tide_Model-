import unittest
import pandas as pd
from backtest_tide_early_entry import candidates, make_trade
...
class EarlyEntryTests(unittest.TestCase):
    def frame(self):
        f=pd.DataFrame(dict(open_time=pd.date_range('2026-01-01',periods=25,freq='15min',tz='UTC'),
            ...
            atr14=[2.]*25,raw_signal=[True]+[False]*24,
    ...
    def test_failed_confirmation_keeps_early(self):
        ...
        self.assertEqual([x['arm'] for x in t],['early'])
    ...
    def test_next_open_and_common_stop(self):
        ...
        self.assertEqual([x['entry'] for x in t],[101,102])
        self.assertEqual([x['stop'] for x in t],[98,98])
