...
# ============================================================
# Crypto Tide Engine V10.9 Dynamic Risk Runtime
...
PRODUCTION_FIXED_HOLD_BARS = int(
    os.getenv("PRODUCTION_FIXED_HOLD_BARS", "16")
)
SEND_RESEARCH_ALERTS = True
SEND_PRE_ALERTS = False
PRODUCTION_EXIT_METHOD = os.getenv(
    "PRODUCTION_EXIT_METHOD", "structure_fixed4h"
)
ALLOW_DAILY_EXIT_SWITCH = False

# ---------- V10.8 expectancy grading ----------
# A-/A/A+ are production-actionable grades. B/C remain research/watch.
GRADE_MIN_ACTIONABLE = os.getenv("GRADE_MIN_ACTIONABLE", "A-").strip().upper()

# Frozen A+ Golden Rebound definition from the completed research cycle.
APLUS_MIN_VOLUME = float(os.getenv("APLUS_MIN_VOLUME", "2.0"))
APLUS_MAX_H1_STRENGTH = float(os.getenv("APLUS_MAX_H1_STRENGTH", "35.0"))

# A is intentionally broader than A+ but still requires strict production
# quality plus multiple secondary-strength confirmations. These are grading
# features, not new hard entry filters.
A_MIN_NEXT = float(os.getenv("A_MIN_NEXT", "96.0"))
A_MIN_VOLUME = float(os.getenv("A_MIN_VOLUME", "1.5"))
A_MIN_SIGNAL = float(os.getenv("A_MIN_SIGNAL", "78.0"))
A_MIN_RAW = float(os.getenv("A_MIN_RAW", "80.0"))

# Known negative-expectancy context discovered in the HTF factor study.
NEGATIVE_CORE_MAX_VOLUME = float(
    os.getenv("NEGATIVE_CORE_MAX_VOLUME", "0.8")
)

# V10.8.2 validated Next-candle Rescue path
NEXT_RESCUE_MIN_NEXT = float(os.getenv("NEXT_RESCUE_MIN_NEXT", "95.0"))
NEXT_RESCUE_MIN_COMBINED = float(os.getenv("NEXT_RESCUE_MIN_COMBINED", "65.0"))
NEXT_RESCUE_MIN_SIGNAL = float(os.getenv("NEXT_RESCUE_MIN_SIGNAL", "65.0"))
NEXT_RESCUE_MIN_CONFIRMATION = int(os.getenv("NEXT_RESCUE_MIN_CONFIRMATION", "3"))
NEXT_RESCUE_MAX_RISK = float(os.getenv("NEXT_RESCUE_MAX_RISK", "0.02"))

# ---------- V10.8.2 validated B+ secondary edge ----------
# Frozen from the full portfolio/B+ study.
BPLUS_MAX_H1_STRENGTH = float(
    os.getenv("BPLUS_MAX_H1_STRENGTH", "35.0")
)
BPLUS_MIN_VOLUME = float(
    os.getenv("BPLUS_MIN_VOLUME", "2.0")
)
BPLUS_MIN_CONFIRMATION = int(
    os.getenv("BPLUS_MIN_CONFIRMATION", "3")
)
BPLUS_MIN_COMBINED = float(
    os.getenv("BPLUS_MIN_COMBINED", "70.0")
)
BPLUS_MIN_NEXT = float(
    os.getenv("BPLUS_MIN_NEXT", "90.0")
)
BPLUS_MAX_NEXT_EXCLUSIVE = float(
    os.getenv("BPLUS_MAX_NEXT_EXCLUSIVE", "95.0")
)
BPLUS_MIN_SIGNAL = float(
    os.getenv("BPLUS_MIN_SIGNAL", "70.0")
)

GRADE_EXPECTANCY_REFERENCE = {
    "A+": {
        "label": "STRONGLY RECOMMENDED",
        "expectancy": 0.0109,
        "note": "A+ Fixed-4h final validation; 54 trades, bootstrap 95% CI about +0.53% to +1.63%.",
    },
    "A": {
        "label": "RECOMMENDED",
        "expectancy": 0.0060,
        "note": "Positive secondary-filter evidence; less robust than A+.",
    },
    "A-": {
        "label": "CAN TAKE",
        "expectancy": 0.0051,
        "note": (
            "Includes strict base-system entries and validated Next Rescue. "
            "Next Rescue full rescan: 60 rescued trades, ~+0.51%/trade overall; "
            "train +0.36%, validation +0.82%."
        ),
    },
    "B+": {
        "label": "POSITIVE EDGE / SECONDARY SETUP",
        "expectancy": 0.003435,
        "note": (
            "Validated secondary edge: 60 trades, +0.3435%/trade overall, "
            "PF 1.54; recent validation 19 trades, +0.742%/trade, PF 2.60. "
            "Lower priority than A-/A/A+."
        ),
    },
    "B": {
        "label": "WATCH / EDGE NOT PROVEN",
        "expectancy": 0.0000,
        "note": "Near-miss or mixed evidence; Telegram suppressed in V10.8.2.",
    },
    "C": {
        "label": "NEGATIVE EXPECTANCY / AVOID",
        "expectancy": -0.0053,
        "note": "Negative evidence in tested weak-volume downtrend contexts.",
    },
}
...
HEALTH_STATE_JSON = DATA_DIR / "daily_health_state.json"
...
def _new_health_state() -> dict:
    return {
        "period_start_utc": datetime.now(timezone.utc).isoformat(),
        "last_report_sydney_date": None,
        "counts": {
            "raw_candidates": 0,
            "scored_setups": 0,
            "A+": 0,
            ...
            "B": 0,
            "C": 0,
            "telegram_trade_alerts": 0,
        },
        "best_rejected": None,
    }

health_state = load_json(HEALTH_STATE_JSON, _new_health_state())

def _save_health_state() -> None:
    save_json(HEALTH_STATE_JSON, health_state)

def health_inc(key: str, amount: int = 1) -> None:
    with health_lock:
        counts = health_state.setdefault("counts", {})
        counts[key] = int(counts.get(key, 0)) + int(amount)
        _save_health_state()

def health_note_grade(
    symbol: str,
    latest: pd.Series,
    score: float,
    assessment: dict,
    ...
    with health_lock:
        counts = health_state.setdefault("counts", {})
        counts["scored_setups"] = int(counts.get("scored_setups", 0)) + 1
        grade_name = str(grade.get("grade", "B"))
        counts[grade_name] = int(counts.get(grade_name, 0)) + 1

        if grade_name in {"B", "C"}:
            nxt = float(assessment.get("next_quality", 0.0))
            combined = float(assessment.get("combined_quality", 0.0))
            diagnostic_score = (
                0.45 * min(nxt / 95.0, 1.10)
                + 0.30 * min(combined / 70.0, 1.10)
                + 0.25 * min(float(score) / 70.0, 1.10)
            )
            candidate = {
                "symbol": symbol,
                "grade": grade_name,
                "raw": float(latest.get("raw_quality_score", np.nan)),
                "next": nxt,
                "combined": combined,
                "signal": float(score),
                "confirmation": int(assessment.get("confirmation_tests", 0)),
                "reason": " | ".join(grade.get("reasons", [])[:3]),
                "diagnostic_score": float(diagnostic_score),
            }
            best = health_state.get("best_rejected")
            if best is None or diagnostic_score > float(best.get("diagnostic_score", -1)):
                health_state["best_rejected"] = candidate

        _save_health_state()

def daily_health_message() -> str:
    with health_lock:
        counts = dict(health_state.get("counts", {}))
        best = health_state.get("best_rejected")
        period_start = health_state.get("period_start_utc")

    lines = [
        "🌊 TIDE DAILY HEALTH",
        "",
        "Scanner: ONLINE",
        f"Period start UTC: {period_start}",
        "Counters: persistent across 6h watchlist restarts",
        f"Raw candidates: {counts.get('raw_candidates', 0)}",
        f"Scored setups: {counts.get('scored_setups', 0)}",
        "",
        f"A+: {counts.get('A+', 0)}",
        f"A: {counts.get('A', 0)}",
        f"A-: {counts.get('A-', 0)}",
        ...
        f"C: {counts.get('C', 0)}",
        "",
        f"Telegram trade alerts: {counts.get('telegram_trade_alerts', 0)}",
    ]
    if best:
        lines += [
            "",
            "Best rejected candidate:",
            f"{best.get('symbol')} — grade {best.get('grade')}",
            f"Raw: {float(best.get('raw', np.nan)):.1f}",
            f"Next: {float(best.get('next', np.nan)):.1f}",
            f"Combined: {float(best.get('combined', np.nan)):.1f}",
            f"Signal: {float(best.get('signal', np.nan)):.1f}",
            f"Confirmation: {int(best.get('confirmation', 0))}/3",
            f"Reason: {best.get('reason', '')}",
        ]
    return "\n".join(lines)

def maybe_send_daily_health() -> None:
    """Send once daily around 07:55 Sydney time, before the 08:00 full rescan."""
    local_now = datetime.now(LOCAL_TIMEZONE)
    today_key = local_now.date().isoformat()

    # The monitor loop runs every 60s.
    # Send only in a narrow Sydney-morning window. Do NOT treat every startup
    # after 08:00 as a missed report, otherwise the 6-hour watchlist restart
    # produces an empty Health message.
    minutes_now = local_now.hour * 60 + local_now.minute
    report_window = (7 * 60 + 55) <= minutes_now < (8 * 60)
    if not report_window:
        return

    with health_lock:
        if health_state.get("last_report_sydney_date") == today_key:
            return

    send_tg(daily_health_message())

    with health_lock:
        health_state["last_report_sydney_date"] = today_key
        health_state["period_start_utc"] = datetime.now(timezone.utc).isoformat()
        health_state["counts"] = {
            "raw_candidates": 0,
            "scored_setups": 0,
            "A+": 0,
        ...
        health_state["best_rejected"] = None
        _save_health_state()
...
STRATEGY_SPECS = {
    "structure_fixed4h": StrategySpec(
        "structure_fixed4h", "structure", "fixed4h"
    ),
...
def run_exit_method(df: pd.DataFrame, entry_idx: int, method: str) -> ExitResult | None:
    ...
    for idx in range(entry_idx+1, last+1):
        ...
        if spec.exit_mode == "fixed4h" and idx-entry_idx >= 16:
            return ExitResult(idx, close, "fixed_4h")
        if spec.exit_mode == "fixed3_5h" and idx-entry_idx >= 14:
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V10.8.2.2 all-eligible watchlist refreshed\n\n"
...
def dynamic_position_size(score: float, risk: dict) -> dict:
    ...
    # Never raise a wide-stop trade to the minimum notional: doing so would
    # exceed the risk budget. Wide structural stops therefore shrink size.
    notional = float(min(uncapped_notional, MAX_POSITION_NOTIONAL_USDT))
    below_display_minimum = notional < MIN_POSITION_NOTIONAL_USDT
...
def active_open_risk_status() -> dict:
    active = [
        ...
        if (
            ...
            and position.get("tracking_class", "production") == "production"
        )
    ]
    open_risk = sum(
...
def open_research_position(
    ...
    components: dict,
    meta: dict,
    tracking_class: str = "production",
    classification_label: str = "SYSTEM-APPROVED",
    ...
    with position_lock:
        ...
        if existing and existing.get("status") == "active":
            log(
                symbol,
                "Tracking position not opened because an active position "
                "already exists",
                existing.get("tracking_class", "production"),
            )
            return False

        method = method_override or symbol_exit_method(symbol)
        spec = strategy_spec(method)
        ...
        is_production = tracking_class == "production"

        active_positions[symbol] = {
            "status": "active",
            "tracking_class": tracking_class,
            "system_entry": is_production,
            "classification_label": classification_label,
            "rejection_reasons": rejection_reasons or [],
            "tide_grade": (
                (grade_info or {}).get("grade", "B")
            ),
            "grade_label": (
                (grade_info or {}).get("label", classification_label)
            ),
            "grade_expectancy_reference": (
                (grade_info or {}).get("expectancy_reference")
            ),
            "h4_trend": (grade_info or {}).get("h4_trend"),
            "h1_trend": (grade_info or {}).get("h1_trend"),
            "h1_strength": (grade_info or {}).get("h1_strength"),
            "h4_strength": (grade_info or {}).get("h4_strength"),
            "method": method,
            ...
            "account_risk_pct": (
                sizing["account_risk_pct"] if is_production else 0.0
            ),
            "planned_loss_usdt": (
                sizing["planned_loss_usdt"] if is_production else 0.0
            ),
            "effective_planned_loss_usdt": (
                sizing["effective_planned_loss_usdt"]
                if is_production else 0.0
            ),
            "effective_account_risk_pct": (
            ...
            "suggested_margin": (
                sizing["suggested_margin_usdt"] if is_production else 0.0
            ),
            "suggested_leverage": (
                sizing["suggested_leverage"] if is_production else 0.0
            ),
            "suggested_notional": (
                sizing["suggested_notional_usdt"] if is_production else 0.0
            ),
            "leverage_capped_for_stop_safety": (
                sizing["leverage_capped_for_stop_safety"]
...
def close_research_position(
    ...
    append_live_trade({
        "symbol": symbol,
        "tracking_class": position.get("tracking_class", "production"),
        "system_entry": position.get("system_entry", True),
        "classification_label": position.get("classification_label"),
        "tide_grade": position.get("tide_grade"),
        "grade_label": position.get("grade_label"),
        "grade_expectancy_reference": position.get(
            "grade_expectancy_reference"
        ),
        "h4_trend": position.get("h4_trend"),
        "h1_trend": position.get("h1_trend"),
        "h1_strength": position.get("h1_strength"),
        "h4_strength": position.get("h4_strength"),
        "rejection_reasons": " | ".join(
            position.get("rejection_reasons", [])
        ),
    ...
    tracking_class = position.get(
        "tracking_class", "production"
    )
    is_watch = tracking_class == "watch"
    is_bplus = position.get("tide_grade") == "B+"
    exit_icon = "🟠" if is_watch else "🏁"
    exit_title = (
        "Tide B+ SECONDARY EXIT"
        if is_bplus
        else (
            "Tide WATCH EXIT"
            if is_watch
            else "Tide SYSTEM EXIT"
        )
    )
    system_entry_text = "NO" if is_watch else "YES"
    original_label = position.get(
        "classification_label",
        "WATCH" if is_watch else "SYSTEM-APPROVED",
    )
    reasons_text = " | ".join(
        position.get("rejection_reasons", [])
    ) or "None"

    send_tg(f"""{exit_icon} Crypto Tide V10.8.2.2 {exit_title}: {symbol}

Original classification: {original_label}
System entry: {system_entry_text}
Tracking class: {tracking_class.upper()}

Exit strategy: {position['method']}
Exit reason: {reason}
Reference entry: {entry:.8g}
Reference exit: {exit_price:.8g}
...
Strict-filter failures at entry:
{reasons_text}

{'Research/watch exit only — not a system-approved trade.' if is_watch else 'System-approved tracked exit.'}
This is a model alert, not an exchange order.""")
...
def realtime_exit_update(symbol: str, latest: pd.Series) -> None:
    with position_lock:
        ...
        if spec.exit_mode=="fixed4h" and bars>=16:
            exit_price,reason=close,"fixed_4h"
        elif spec.exit_mode=="fixed3_5h" and bars>=14:
...
def production_entry_assessment(
    ...
    raw_quality = float(latest.get("raw_quality_score", 0.0))

    failures = []
    if raw_quality < MIN_RAW_QUALITY_SCORE:
        failures.append(
            f"Raw quality {raw_quality:.1f} < "
            f"{MIN_RAW_QUALITY_SCORE:.1f}"
        )
    ...
    # V10.9 Dynamic Risk: stop distance is NOT a signal-quality rejection gate.
    # The structural stop stays where the setup is invalidated; position size
    # is reduced so account-level planned loss remains controlled.
...
def _htf_trend_snapshot(df: pd.DataFrame, timeframe_hours: int, signal_close: pd.Timestamp) -> dict:
    """Return a completed-candle EMA trend snapshot using the exact research definition."""
    ...
    candle_end = work["open_time"] + pd.Timedelta(hours=timeframe_hours)
    work = work[candle_end <= signal_close].copy()
    if len(work) < 205:
        return {
            "trend": "MIXED",
            "strength": 50.0,
    ...
    slope3 = ema50.pct_change(3)

    c = float(close.iloc[-1])
    e50 = float(ema50.iloc[-1])
    e200 = float(ema200.iloc[-1])
    s3 = float(slope3.iloc[-1])

    if c > e50 and e50 > e200 and s3 > 0:
        trend = "UP"
    elif c < e50 and e50 < e200 and s3 < 0:
        trend = "DOWN"
    else:
        trend = "MIXED"
    ...
    strength = float(np.clip(
        50
        + 20 * np.tanh((0.0 if not np.isfinite(s3) else s3) * 200)
        + 15 * np.tanh((c / e50 - 1.0) * 50)
        + 15 * np.tanh((e50 / e200 - 1.0) * 30),
        0,
        100,
    ))
...
def live_htf_context(symbol: str, latest: pd.Series) -> dict:
    """Compute 1H/4H context only when a scored 15m setup closes."""
    signal_close = latest["open_time"] + pd.Timedelta(minutes=15)

    with data_lock:
        df1h = market_data.get(symbol, {}).get("60", pd.DataFrame()).copy()

    h1 = _htf_trend_snapshot(df1h, 1, signal_close)

    try:
        # REST fetch only on actual scored setups; avoids streaming 4H for the whole universe.
        df4h = fetch_klines(symbol, "240", 90)
        h4 = _htf_trend_snapshot(df4h, 4, signal_close)
    except Exception as exc:
        log(symbol, "4H context fetch failed", repr(exc))
        h4 = {
            "trend": "MIXED",
            "strength": 50.0,
            "close": np.nan,
...
def tide_grade(
    ...
    """Assign a V10.8 grade from frozen research evidence.

    A-/A/A+ require the strict production-quality gate. This keeps grading
    separate from the raw signal score and prevents a strong context from
    rescuing a weak 15m setup.
    """
    strict = bool(assessment["production"])
    raw = float(latest.get("raw_quality_score", 0.0))
    nxt = float(assessment["next_quality"])
    combined = float(assessment["combined_quality"])
    conf = int(assessment["confirmation_tests"])
    volume = float(latest.get("volume_multiple", 0.0))
    risk_pct = float(assessment["risk"]["risk_pct"])

    core_downtrend = (
        htf.get("h4_trend") == "DOWN"
        and htf.get("h1_trend") == "DOWN"
    )

    aplus = (
        strict
        and core_downtrend
        and volume >= APLUS_MIN_VOLUME
        and float(htf.get("h1_strength", 50.0)) <= APLUS_MAX_H1_STRENGTH
    )

    # A requires strict quality plus at least two independently researched
    # strength confirmations. This is deliberately broad and is not allowed
    # to override the A+ frozen rule.
    a_strength_votes = sum([
        nxt >= A_MIN_NEXT,
        volume >= A_MIN_VOLUME,
        score >= A_MIN_SIGNAL,
        raw >= A_MIN_RAW,
    ...
    if aplus:
        ...
        reasons = [
            "4H DOWN + 1H DOWN",
            f"Volume {volume:.2f}x >= {APLUS_MIN_VOLUME:.1f}x",
            f"H1 strength {float(htf.get('h1_strength', 50.0)):.1f} <= {APLUS_MAX_H1_STRENGTH:.0f}",
            "All strict entry filters passed",
        ]
    elif strict and a_strength_votes >= 3:
        grade = "A"
        reasons = [
            f"{a_strength_votes}/5 secondary strength confirmations",
            "All strict entry filters passed",
        ]
    elif strict:
        grade = "A-"
        reasons = [
            "All strict entry filters passed",
            "No stronger A/A+ context confirmation",
        ]
    else:
        next_rescue = (
            nxt >= NEXT_RESCUE_MIN_NEXT
            and combined >= NEXT_RESCUE_MIN_COMBINED
            and score >= NEXT_RESCUE_MIN_SIGNAL
            and conf >= NEXT_RESCUE_MIN_CONFIRMATION
        )
        if next_rescue:
            grade = "A-"
            reasons = [
                "NEXT RESCUE validated entry",
                f"Next {nxt:.1f} >= {NEXT_RESCUE_MIN_NEXT:.1f}",
                f"Combined {combined:.1f} >= {NEXT_RESCUE_MIN_COMBINED:.1f}",
                f"Signal {score:.1f} >= {NEXT_RESCUE_MIN_SIGNAL:.1f}",
                f"Confirmation {conf}/3 >= {NEXT_RESCUE_MIN_CONFIRMATION}/3",
                f"Structural stop distance {risk_pct:.2%}; size adjusts dynamically",
                "Raw Tide may fail the normal strict threshold",
            ]
        elif (
            core_downtrend
            and float(htf.get("h1_strength", 50.0)) <= BPLUS_MAX_H1_STRENGTH
            and volume >= BPLUS_MIN_VOLUME
            and conf >= BPLUS_MIN_CONFIRMATION
            and combined >= BPLUS_MIN_COMBINED
            and nxt >= BPLUS_MIN_NEXT
            and nxt < BPLUS_MAX_NEXT_EXCLUSIVE
            and score >= BPLUS_MIN_SIGNAL
        ):
            grade = "B+"
            reasons = [
                "VALIDATED B+ secondary edge",
                "4H DOWN + 1H DOWN",
                f"H1 strength {float(htf.get('h1_strength', 50.0)):.1f} <= {BPLUS_MAX_H1_STRENGTH:.0f}",
                f"Volume {volume:.2f}x >= {BPLUS_MIN_VOLUME:.1f}x",
                f"Confirmation {conf}/3 >= {BPLUS_MIN_CONFIRMATION}/3",
                f"Combined {combined:.1f} >= {BPLUS_MIN_COMBINED:.1f}",
                f"Next {nxt:.1f} in [{BPLUS_MIN_NEXT:.1f}, {BPLUS_MAX_NEXT_EXCLUSIVE:.1f})",
                f"Signal {score:.1f} >= {BPLUS_MIN_SIGNAL:.1f}",
                "Secondary setup; lower priority than A-/A/A+",
            ]
        elif core_downtrend and volume < NEGATIVE_CORE_MAX_VOLUME:
            grade = "C"
            reasons = [
                "4H DOWN + 1H DOWN with weak volume",
                f"Volume {volume:.2f}x < {NEGATIVE_CORE_MAX_VOLUME:.1f}x",
                "This tested context had negative historical expectancy",
            ]
        else:
            grade = "B"
            reasons = assessment["failures"][:] or [
                "Strict edge not established",
            ]

    ref = GRADE_EXPECTANCY_REFERENCE[grade]
    actionable = grade in {"A+", "A", "A-"}
    secondary_actionable = grade == "B+"
    telegram_eligible = grade in {"A+", "A", "A-", "B+"}

    return {
        "grade": grade,
        "label": ref["label"],
        "expectancy_reference": float(ref["expectancy"]),
        "expectancy_note": ref["note"],
        "actionable": actionable,
        "secondary_actionable": secondary_actionable,
        "telegram_eligible": telegram_eligible,
        "core_downtrend": core_downtrend,
        "reasons": reasons,
        "a_strength_votes": a_strength_votes,
        **htf,
    }
...
def grade_rank(grade: str) -> int:
    return {
        ...
        "A": 4,
        "A+": 5,
    }.get(str(grade).upper(), 0)


def quality_mark(value: float, threshold: float, near_margin: float = 0.0) -> str:
    if value >= threshold:
        return "✅ PASS"
    if near_margin > 0 and value >= threshold - near_margin:
        return "⚠️ NEAR"
    return "❌ FAIL"


def classify_quality_signal(
    latest: pd.Series,
    score: float,
    assessment: dict,
) -> tuple[str, str]:
    failures = assessment["failures"]
    next_quality = assessment["next_quality"]
    combined = assessment["combined_quality"]
    confirmations = assessment["confirmation_tests"]
    risk_pct = assessment["risk"]["risk_pct"]
    ...
    near_failure = (
        len(failures) == 1
        and (
            next_quality >= PRODUCTION_MIN_NEXT_QUALITY - 5
            or combined >= PRODUCTION_MIN_COMBINED_QUALITY - 5
            or score >= assessment["required_score"] - 5
            or confirmations >= max(0, PRODUCTION_MIN_CONFIRMATION_TESTS - 1)
            or risk_pct <= PRODUCTION_MAX_HARD_STOP_RISK_PCT + 0.005
        )
    )
    if near_failure:
        return "🟠", "NEAR MISS"
    ...
    passed_core = sum([
        ...
        confirmations >= 1,
        score >= 60,
        float(latest.get("raw_quality_score", 0.0)) >= MIN_RAW_QUALITY_SCORE,
    ])
    if passed_core >= 3:
        return "🟡", "WATCH / MIXED QUALITY"

    return "🔴", "LOW QUALITY / NOT RECOMMENDED"


def send_quality_signal_alert(
    symbol: str,
    latest: pd.Series,
    score: float,
    meta: dict,
    ...
    """V10.8.2: alert only B+ or higher; auto-track Core and B+ separately."""
    signal_key = latest["open_time"].isoformat()
    if already_alerted(symbol, "quality_signal", signal_key):
        return False

    risk = assessment["risk"]
    raw_quality = float(latest.get("raw_quality_score", 0.0))
    next_quality = assessment["next_quality"]
    combined_quality = assessment["combined_quality"]
    confirmations = assessment["confirmation_tests"]
    required_score = assessment["required_score"]
    volume_multiple = float(latest.get("volume_multiple", 0.0))
    lower_wick = float(latest.get("lower_wick_ratio", 0.0))

    htf = live_htf_context(symbol, latest)
    grade = tide_grade(
        latest,
        score,
        assessment,
    ...
    route_name = (
        "NEXT_RESCUE"
        if any("NEXT RESCUE" in str(item) for item in grade.get("reasons", []))
        else (
            "BPLUS"
            if grade.get("grade") == "B+"
            else ("CORE" if grade.get("actionable") else "FILTERED")
        )
    )
    log(
        symbol,
        ...
        f"route={route_name}",
        f"raw={float(latest.get('raw_quality_score', np.nan)):.1f}",
        f"next={assessment['next_quality']:.1f}",
        f"combined={assessment['combined_quality']:.1f}",
        f"signal={score:.1f}",
        f"confirmation={assessment['confirmation_tests']}/3",
        f"volume={float(latest.get('volume_multiple', np.nan)):.2f}x",
        f"h4={grade.get('h4_trend')}",
        f"h1={grade.get('h1_trend')}",
        f"h1_strength={float(grade.get('h1_strength', np.nan)):.1f}",
    )
    health_note_grade(symbol, latest, score, assessment, grade)

    # V10.8.2 notification floor: B+.
    # B and C are still internally graded/audited, but no Telegram message
    # and no realtime watch position is opened for them.
    if not grade.get("telegram_eligible", False):
        log(
            symbol,
            "Telegram suppressed",
            f"grade={grade['grade']}",
    ...
    actionable = bool(grade["actionable"])
    secondary_actionable = bool(
        grade.get("secondary_actionable", False)
    )
    ...
    if actionable:
        ...
        if not capacity_allowed:
            capacity_note = (
                ...
                f"Reason: {', '.join(capacity['reasons'])}\n"
            )

    raw_mark = quality_mark(
        raw_quality, MIN_RAW_QUALITY_SCORE, near_margin=3.0
    )
    next_mark = quality_mark(
        next_quality, PRODUCTION_MIN_NEXT_QUALITY, near_margin=5.0
    )
    combined_mark = quality_mark(
        combined_quality, PRODUCTION_MIN_COMBINED_QUALITY, near_margin=5.0
    )
    confirmation_mark = (
        "✅ PASS"
        if confirmations >= PRODUCTION_MIN_CONFIRMATION_TESTS
        else (
            "⚠️ NEAR"
            if confirmations == PRODUCTION_MIN_CONFIRMATION_TESTS - 1
            else "❌ FAIL"
        )
    )
    score_mark = quality_mark(
        score, required_score, near_margin=5.0
    )
    risk_mark = "📐 DYNAMIC SIZE"

    failure_lines = (
        "\n".join(f"• {item}" for item in assessment["failures"])
        if assessment["failures"]
        else "• None — strict entry filters passed."
    )
    grade_reason_lines = "\n".join(
        f"• {item}" for item in grade["reasons"]
    )
    entry_route = (
        "NEXT RESCUE"
        if any("NEXT RESCUE" in str(item) for item in grade["reasons"])
        else (
            "SECONDARY B+"
            if secondary_actionable
            else "STRICT / STANDARD"
        )
    )

    planned_exit = (
        "Fixed 4h (16 completed 15m bars), unless stop is hit first"
        if (actionable or secondary_actionable)
        else "No realtime tracking"
    )

    health_inc("telegram_trade_alerts")
    send_tg(f"""{grade_icon} Tide SIGNAL: {symbol}

TIDE GRADE: {grade['grade']} — {grade['label']}
Entry route: {entry_route}
Default system trade: {'YES' if actionable else ('OPTIONAL / SECONDARY' if secondary_actionable else 'NO')}
Production position opened: {'YES' if actionable and capacity_allowed else 'NO'}
Secondary B+ tracking: {'YES' if secondary_actionable else 'NO'}

Backtest expectancy reference: {grade['expectancy_reference']:+.2%} / trade
Evidence note: {grade['expectancy_note']}

Candle close UTC: {latest['open_time'] + pd.Timedelta(minutes=15)}
Reference/current price: {latest['close']:.8g}

HIGHER-TIMEFRAME CONTEXT
4H trend: {grade['h4_trend']}
4H strength: {grade['h4_strength']:.1f}/100
1H trend: {grade['h1_trend']}
1H strength: {grade['h1_strength']:.1f}/100

QUALITY CHECK
Raw Tide: {raw_quality:.1f}/100 — {raw_mark}
Reference: ≥ {MIN_RAW_QUALITY_SCORE:.1f}

Next candle: {next_quality:.1f}/100 — {next_mark}
Strict reference: ≥ {PRODUCTION_MIN_NEXT_QUALITY:.1f}

Combined setup: {combined_quality:.1f}/100 — {combined_mark}
Strict reference: ≥ {PRODUCTION_MIN_COMBINED_QUALITY:.1f}

Confirmation: {confirmations}/3 — {confirmation_mark}
Strict reference: ≥ {PRODUCTION_MIN_CONFIRMATION_TESTS}/3

Signal score: {score:.1f}/100 — {score_mark}
Required score: ≥ {required_score:.1f}

Volume multiple: {volume_multiple:.2f}x
Lower wick ratio: {lower_wick:.4f}

RISK / EXIT REFERENCE
Entry reference: {risk['entry_price']:.8g}
Signal low: {risk['signal_low']:.8g}
Stop: {risk['hard_stop']:.8g}
Stop risk: {risk['risk_pct']:.2%} — {risk_mark}
Planned exit: {planned_exit}

WHY THIS GRADE
{grade_reason_lines}

STRICT-FILTER FAILURES
{failure_lines}
{capacity_note}
GRADE INTERPRETATION
A+ = strongest tested edge / strongly recommended
A  = recommended
A- = Core; strict base-system edge OR validated Next Rescue
B+ = validated positive secondary edge; lower priority than Core
B  = below Telegram threshold / edge not proven
C  = below Telegram threshold / tested negative-expectancy context

Tracking:
• A-/A/A+ open a SYSTEM tracked position when portfolio capacity allows.
• B+ opens a SECONDARY/WATCH tracked position and receives stop/Fixed-4h exit alerts.
• B+ consumes zero Core production portfolio risk, so it cannot crowd out A-/A/A+.
• B/C generate no Telegram signal and no realtime watch position.

This is model research/decision support, not an exchange order.""")

    components = signal_components(
        latest,
        meta.get("historical_score"),
    )

    if actionable and capacity_allowed:
        return open_research_position(
            symbol,
            latest,
            score,
            components,
            ...
            tracking_class="production",
            classification_label=(
                f"{grade['grade']} {grade['label']}"
            ),
            rejection_reasons=[],
            grade_info=grade,
            method_override="structure_fixed4h",
        )
    ...
    if secondary_actionable:
        return open_research_position(
            ...
            components,
            meta,
            tracking_class="watch",
            classification_label=(
                f"{grade['grade']} {grade['label']}"
            ),
            rejection_reasons=assessment["failures"],
            grade_info=grade,
            method_override="structure_fixed4h",
        )

    # Core signal may be portfolio-blocked: do not silently downgrade it
    # into B+ watch tracking. The Telegram message already explains the block.
    return False
...
def calculate_live_signal(symbol: str, confirmed: bool) -> None:
    ...
    if confirmed:
        ...
        if has_scored_setup:
            ...
            position_opened = send_quality_signal_alert(
                symbol,
                latest,
                float(score),
                meta,
            ...
            opened_position = active_positions.get(symbol, {})
            opened_class = opened_position.get("tracking_class")

            if position_opened and opened_class == "production":
                decision = "graded_production_entry"
                reason = opened_position.get(
                    "classification_label", "A-/A/A+"
                )
            elif position_opened and opened_class == "watch":
                decision = "graded_watch_position"
                reason = opened_position.get(
                    "classification_label", "B/C"
                )
            elif assessment["production"]:
                decision = "qualified_but_not_opened"
                reason = "portfolio_blocked_or_existing_position"
            else:
                decision = "watch_signal_not_opened"
                reason = (
            ...
            append_setup_audit({
                ...
                "tide_grade": active_positions.get(symbol, {}).get(
                    "tide_grade"
                ),
                "grade_label": active_positions.get(symbol, {}).get(
                    "grade_label"
                ),
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V10.8.2.2 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V10.9 Dynamic Risk monitor started\n\n"
