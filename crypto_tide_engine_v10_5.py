...
# ============================================================
# Crypto Tide Engine V10.5 Dynamic Risk Runtime Strategy.2
...
# V10.5 sizes each position from the model stop distance rather than using a
...
def scan_and_select() -> tuple[list[str], pd.DataFrame]:
    ...
    lines = [
        "✅ Crypto Tide Engine V10.5 daily all-eligible learning complete",
        "",
...
def select_current_watchlist(results: pd.DataFrame) -> list[str]:
    ...
    send_tg(
        "🔄 Crypto Tide V10.5 all-eligible watchlist refreshed\n\n"
...
def close_research_position(
    ...
    send_tg(f"""🏁 Crypto Tide V10.5 EXIT ALERT: {symbol}
...
def realtime_exit_update(symbol: str, latest: pd.Series) -> None:
    with position_lock:
        ...
        previous_lowest = float(position.get("lowest_price", entry))
        highest = max(previous_highest, high)
        lowest = min(previous_lowest, low)
        mfe_pct = highest / entry - 1.0
        mae_pct = lowest / entry - 1.0
        ...
        if spec.stop_mode != "none" and low <= hard_stop:
            ...
            position["maximum_favourable_excursion_pct"] = mfe_pct
            position["maximum_adverse_excursion_pct"] = mae_pct
        ...
        if (
            ...
            send_tg(f"""⚠️ Crypto Tide V10.5 LOGIC WARNING: {symbol}
    ...
    if confirmed:
        ...
        if confirmed_signal:
            ...
            register_pending_setup_outcomes(
                symbol, latest, "confirmed_entry", score, "model_confirmed"
            )
            append_setup_audit({
                "event_type": "decision",
                "symbol": symbol,
        ...
        elif has_scored_setup:
            reason = "model_score_or_setup_threshold_rejected"
            register_pending_setup_outcomes(
                symbol, latest, "rejected", score, reason
            )
            append_setup_audit({
                "event_type": "decision",
                "symbol": symbol,
...
def start_monitor(symbols: list[str]) -> None:
    if not symbols:
        send_tg(
            "⚠️ Crypto Tide V10.5 found no robust-eligible symbols.\n"
    ...
    send_tg(
        "✅ Crypto Tide Engine V10.5 realtime monitor started\n\n"
