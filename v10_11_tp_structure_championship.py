...
# -*- coding: utf-8 -*-
"""
V10.11 TP + STRUCTURE STOP CHAMPIONSHIP
============================================
...
def parse_args():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("v10_11_tp_structure_championship"),
    )
...
def main():
    ...
    print("V10.11 TP + STRUCTURE STOP CHAMPIONSHIP", flush=True)
    print("=" * 100, flush=True)
    ...
    for exit_method in sorted(cand["exit_method"].unique()):
        ...
        for name, grades in policies:
            ...
            summary["tp_pct"] = float(method_cand["tp_pct"].iloc[0])
    ...
    tp_rows = []
    for (tp_pct, exit_method), g in cand.groupby(["tp_pct", "exit_method"]):
        r = g["net_return"].astype(float)
    ...
    grade_tp_rows = []
    for (grade, tp_pct), g in cand.groupby(["grade", "tp_pct"]):
        r = g["net_return"].astype(float)
        gains = float(r[r > 0].sum())
        losses = float(-r[r < 0].sum())
